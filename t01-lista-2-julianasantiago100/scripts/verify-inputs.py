...
for file in glob.glob( input_dir + '/*.txt' ):
    ...
    #timeout file is optional per test
    expected_timeout_file = expected_dir + '/' + file_name + '_TIME' + ext_name 
    ...
    timeout = 2 #default timeout is 2 seconds
    try:
        with open(expected_timeout_file) as f: #timout file is optional per test
    ...
    # timeout    - seconds for program to timeout, if execution is more than that, infinite loop is supposed
    try:
        result = run([executable], capture_output=True, text=True, input=input, timeout=timeout)
    except TimeoutExpired:
        print('\033[1;31m[%s]: Loop infinito durante a execução do programa (mais de %d segs).\033[0;0m' % (test_name, timeout))
        print("\033[1;31m> Entrada:\n=------[ início ]------=\n%s\n=------[ fim ]---------=\n\033[0;0m" % (input))
        n_runtime_errors +=1
        continue # Go to the next test case.
